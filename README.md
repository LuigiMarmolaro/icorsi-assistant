...
- Local OCR (tesseract.js) for scanned PDFs
- Sentence-level highlighting inside a custom PDF viewer
- Forum thread indexing (`mod/forum`) — with care around other students' data
...
- **No aggressive scraping.** Deliberately conservative limits: max 20 documents
  per scan, 2 parallel downloads, a 300 ms pause between downloads, scanning
  only on explicit user action. Do not mass-crawl the whole platform.
- **Terms of use.** Check the terms of service of iCorsi / your university: some
  institutions forbid automated tools even for personal use. Course materials
  are copyrighted: local indexing for personal study is generally comparable to
  saving the PDFs on your own computer, but **do not redistribute** the contents
  or the index.
- **Local data, local processing.** Documents, chunks and settings stay in the
  browser (IndexedDB + chrome.storage.local); answers are generated by Ollama on
  your machine. No data is sent to external services. The "Clear index" button
  removes everything; removing the extension also removes its storage.
- **Origin-header rewrite.** The declarativeNetRequest rule applies only to
  requests to `localhost:11434` / `127.0.0.1:11434` and requires the matching
  host_permissions: it does not touch traffic to other sites. Ollama listens on
  localhost only, so no external service can reach it.
- **Third-party personal data.** Even though everything stays local, avoid
  indexing pages with other students' data (participant lists, forums with
  names) — they are not needed for the tool's purpose.
